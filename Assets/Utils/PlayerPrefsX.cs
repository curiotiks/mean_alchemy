...
public class PlayerPrefsX
{
    ...
    public static float GetFloat(String name, float num=1f)
    {
        return PlayerPrefs.GetFloat(name, num);
    }
    ...
    public static bool SetBool(String name, bool value)
    {
        try
        {
            PlayerPrefs.SetInt(name+ "_bool", value ? 1 : 0);
            PlayerPrefs.Save();
        }
        catch
        {
    ...
    {
        return PlayerPrefs.GetInt(name + "_bool") == 1;
    }

    public static bool GetBool(String name, bool defaultValue)
    {
        return (1 == PlayerPrefs.GetInt(name+ "_bool", defaultValue ? 1 : 0));
    }
    ...
    //이름도 배열로 관리한다
    //kinds = normal, connection
    public static void setNameWithArray(string kinds, string name, string[] array)
    {
        if (kinds.Contains("normal"))
        {
            string[] temp = GetStringArray("normalNameArray");
            List<string> list = new List<string>();
            list.AddRange(temp);
            if (!list.Contains(name))
            {
                list.Add(name);
                SetStringArray("normalNameArray", list.ToArray());
                SetStringArray("normal"+name, array);
            }
        }
        else if (kinds.Contains("connection"))
        {
            string[] temp = GetStringArray("connectionNameArray");
            List<string> list = new List<string>();
            list.AddRange(temp);
            if (!list.Contains(name))
            {
                list.Add(name);
                SetStringArray("connectionNameArray", list.ToArray());
                SetStringArray("connection"+name, array);
            }
        }
        PlayerPrefs.Save();
    }

    public static void deleteCubeByNameWithArray(string kinds, string name)
    {
        if (kinds.Contains("normal"))
        {
            string[] temp = GetStringArray("normalNameArray");
            ...
            if (list.Contains(name))
            {
                list.Remove(name);
                SetStringArray("normalNameArray", list.ToArray());
                PlayerPrefs.DeleteKey("normal"+name);
            }
        }
        else if (kinds.Contains("connection"))
        {
            ...
            if (list.Contains(name))
            {
                list.Remove(name);
                SetStringArray("connectionNameArray", list.ToArray());
                PlayerPrefs.DeleteKey("connection"+name);
            }
        }
        PlayerPrefs.Save();
    }
